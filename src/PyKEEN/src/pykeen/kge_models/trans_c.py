...
"""Implementation of the TransC model."""
...
from pykeen.constants import NORM_FOR_NORMALIZATION_OF_ENTITIES, SCORING_FUNCTION_NORM, TRANS_C_NAME
...
class TransC(BaseModule):
    ...
    def __init__(self, config: Dict) -> None:
        ...
        # type specific loss
        self.margin_loss = self.margin_loss * np.array([1, 1, 1])
        self.criterions = [
            nn.MarginRankingLoss(
                margin=margin,
                size_average=False #  self.margin_ranking_loss_size_average,
            )
            for margin in self.margin_loss
        ]
    ...
    def forward(self, batch_positives, batch_negatives, types):
        ...
        loss = self._compute_loss(positive_scores=positive_scores, negative_scores=negative_scores, triple_types=types)
        return loss

    def _compute_loss(self, positive_scores, negative_scores, triple_types):
        ...
        triple_types = torch.tensor(triple_types, dtype=torch.float, device=self.device)
        losses = [
            self.criterions[i](
                positive_scores[triple_types == i],
                negative_scores[triple_types == i],
                y[triple_types == i]
            ) * torch.tensor(sum(triple_types == i), dtype=torch.float, device=self.device)
            for i in range(3)
        ]
        return sum(losses) # / len(triple_types)
